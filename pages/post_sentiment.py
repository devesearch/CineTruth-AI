...
def display_sentiment_results(result):
    ...
    with col1:
        ...
        if result.get("visual_text"):
            st.subheader("Text Detected from Image / Video")
            st.write(result["visual_text"])

        if result.get("audio_text"):
            st.subheader("Speech Detected from Video")
            st.write(result["audio_text"])

        if result.get("text") and not (result.get("visual_text") or result.get("audio_text")):
